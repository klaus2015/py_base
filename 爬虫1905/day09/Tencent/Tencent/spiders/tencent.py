...
class TencentSpider(scrapy.Spider):
    ...
    one_url = 'https://careers.tencent.com/tencentcareer/api/post/Query?timestamp=1566266592644&countryId=&cityId' \
    ...
    def parse_one_page(self,response):
        # response.text ->json格式字符串
        html = json.loads(response.text)

        for job in html['Data']['Posts']:
            # 放入循环里，多级页面，多个地址操作一个item对象，多线程操作item会发生冲突
            item = TencentItem()
            post_id = job['PostId']
