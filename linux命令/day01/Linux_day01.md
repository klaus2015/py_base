...
4、ls -lh file|directory
  显示文件权限及详细信息,h选项提供易读容量单位
  -rw-r--r--  tarena tarena
  分三组:  所有者权限 所属组用户权限 其他用户权限
  r - 读(4)
  w - 写(2)
  x - 可执行(1)
  前3位: 所有者(tarena这个用户)rw-
  中3位: 所属组其它用户(用户tarena2对文件权限)r--
  尾3位: 其它组其它用户(用户develop1对文件权限)r--
  # 777 rwxrwxrwx
  # 666 rw-rw-rw-
  # 644 rw-r--r--
...
9、chmod 权限 file
  ...
  chmod g+w 文件名 # 同组其它用户增加w权限(group)
  chmod o-r 文件名 # 其它组其它用户减r权限(other)
...
4、把/etc/passwd文件拷贝到 MrRight 目录一份(cp命令)
   # sudo cp /etc/passwd ~/MrRight/
5、在 /home/tarena/MrRight/passwd 文件中筛选 tarena 用户的信息(grep命令)
   # gerp "tarena" ~/MrRight/passwd
6、查看passwd文件的权限,并将其权限修改为所有用户都可读可写但是不可执行(chmod命令)
   # chmod 666 ~/MrRight/passwd
7、将 MrRight 目录打包压缩,MrRight-你的名字.tar.gz
   # cd 
   # tar -zcvf MrRight-杨过.tar.gz MrRight
8、将此压缩包远程复制到 主讲机 | 同桌 计算机的电脑上
  # scp MrRight-杨过.tar.gz tarena@172.40.91.174:/home/tarena/homework/
```
...
# 24、去除重复行,并统计每行出现的次数(相邻行) 不加c只去重，加上c统计相同的出现的次数
  uniq -c
  sort 文件名 | uniq -c
```
...
问题3: 统计每个IP地址的访问次数,输出前10个访问量最大的用户IP
       # awk '{print $1}' access.log | sort | uniq -c | sort  -rn -k 1 | head -10
        	# sort  -rn -k 1 默认升序， r代表reverse反转，n代表按数字大小排序，
```
